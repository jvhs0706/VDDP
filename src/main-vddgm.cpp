...
int main(int argc, char** argv) {
    ...
    float sigma = stof(argv[3]);
    uint prob_nbit = stoi(argv[4]);
    string i_file = argv[5];
    string o_file = argv[6];
    ...
    for (uint i = 0; i < nser; ++ i)
    {
        ...
        G1 com_noise;
        vector<int> noise = DGauss(sigma, prob_nbit, pp, computing_timer, ptimer, vtimer, comm, F_noise, R_noise, com_noise);
