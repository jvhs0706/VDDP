...
def safe_t_to_ber(t: float, i: int):
    try:
        return 1 / (1 + math.exp((1<<i)/t))
...
def vdlm_get_priv_cost(t: float, log_range: int, prec: int):
    ...
    p_list_ = [safe_t_to_ber(t, i) for i in range(log_range)]
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            if not os.path.isfile('logs/vddlm.csv'):
                with open('logs/vddlm.csv', 'w') as f:
                    f.write('nser,dim,orig_eps,orig_l1,vdbm_nb,vdbm_l1,eps,delta,num_bit,setup,computing,proving,verifying,comm,l1,l2\n')

            orig_l1 = 2 * np.exp(args.eps) / (np.exp(2 * args.eps) - 1) * args.dim
            if args.nser == 1:
                pass
            elif args.nser == 2:
                orig_l1 *= 2 * (np.exp(2 * args.eps) + np.exp(1 * args.eps) + 1) / ((np.exp(args.eps) + 1)**2)
            else:
                raise NotImplementedError(f'Unsupported nser: {args.nser}')
            
            vdbm_nb = math.ceil(10 * math.log(2/delta) / (eps ** 2))
            vdbm_l1 = args.dim * math.sqrt(args.nser * vdbm_nb/2) / math.sqrt(math.pi)

            log_str = f'{args.nser},{args.dim},{args.eps},{orig_l1},{vdbm_nb},{vdbm_l1},{eps},{delta},{num_bit},'
